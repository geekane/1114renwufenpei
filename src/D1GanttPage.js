...
const GanttChart = () => {
    ...
    // --- 用户手动改变日期范围 (关键修复：强制重绘) ---
    const handleDateRangeChange = (dates) => {
        if (!dates || dates.length < 2) return;
        
        setViewRange(dates); // 更新 React 状态
        
        const newMin = dates[0].format('YYYY-MM-DD');
        const newMax = dates[1].format('YYYY-MM-DD');
        
        console.log('User changed date range to:', newMin, newMax);

        if (instanceRef.current) {
            // 关键：updateOption 传入 dates 的同时，再次传入 records。
            // 这会强制 VTable 重新计算布局，确保新的日期范围立即生效。
            instanceRef.current.updateOption({
                minDate: newMin,
                maxDate: newMax,
                records: records // 必须重新传递 records 才能触发时间轴刻度的重新生成
            });
        }
    };
